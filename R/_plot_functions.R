...
plotGridSelInd <- function(stk){
  ...
  p_sel <- plotSels(stk) +theme(legend.position = 'none')+ggtitle("")
  
  df <- as.data.frame(Fjuv.over.Fmax(stk))
  ...
  df <- as.data.frame(Fjuv.over.Fadult(stk))
  ...
  p1 <- ggarrange(p_F,p_ssb,p_rec,nrow = 3)
  p2 <- ggarrange(p_sel,p_FjFap,p_FjFad,nrow = 3)
